...
async function run() {
  try {
    ...
    // api for fetching top 6 instructors for popular instructors
  ...
  // const instructorEmails = popularClasses.map((eachClass) => eachClass.instructorEmail);
  // const instructors = await usersCollection.find({ email: { $in: instructorEmails } }).toArray();
  const instructors = await usersCollection.find({ role: 'instructor', email: { $ne: 'instructor@ph.com' } }).limit(6).toArray();
